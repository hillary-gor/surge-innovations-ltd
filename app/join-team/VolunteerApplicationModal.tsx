...
export function VolunteerApplicationModal({ children }: { children: React.ReactNode }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    // Checkbox handling: if not checked, it won't be in formData, so we default it
    ...
    if (!validation.success) {
      ...
      validation.error.issues.forEach((issue) => {
        // use the first element of path as key
    ...
    // 2. Server Submission (Only if valid)
    ...
    if (result.success) {
      ...
      if (result.error?.includes("signed in")) {
        window.location.href = "/auth/signin?next=/join-team";
      }
    }
  }
      ...
      <DialogContent className="sm:max-w-3xl w-[95vw] max-h-[90vh] overflow-y-auto p-6 md:p-8">
        <DialogHeader className="mb-4">
          ...
          <DialogDescription className="text-base">
            Complete this form to apply. You must be signed in to submit.
          </DialogDescription>
        </DialogHeader>
        ...
        <form ref={formRef} action={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input name="firstName" id="firstName" className={errors.firstName ? "border-destructive" : "h-10"} />
              <ErrorMessage message={errors.firstName} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="lastName" className={errors.lastName ? "text-destructive" : ""}>Last Name</Label>
              <Input name="lastName" id="lastName" className={errors.lastName ? "border-destructive" : "h-10"} />
          ...
          <div className="grid md:grid-cols-2 gap-6">
             <div className="space-y-2">
                ...
                <Input name="email" id="email" type="email" className={errors.email ? "border-destructive" : "h-10"} />
          ...
          <div className="space-y-2">
            <div className={`flex items-start space-x-3 pt-2 p-4 rounded-md ${errors.terms ? "bg-destructive/10" : "bg-primary/5"}`}>
              ...
              <label htmlFor="terms" className="text-sm text-muted-foreground leading-tight cursor-pointer">
                I understand this is a volunteer position focused on learning & experience. I commit to being reliable and communicative.
              </label>
            </div>
          ...
          <Button type="submit" size="lg" className="w-full text-base py-6" disabled={isSubmitting}>
            {isSubmitting ? (
              <>
                <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Validating & Submitting...
              </>
            ) : (
              "Submit Application"
            )}
